...
import { calculateExpensesByCategory } from '../../utils/engine.js';
...
export default function CategoryBarChart() {
  ...
  const data = useMemo(() => {
    const totals = calculateExpensesByCategory(state);
  ...
  if (data.length === 0) {
    return <EmptyState icon={BarChart3} message="No category data to display" />;
  }
  ...
  return (
    <ResponsiveContainer width="100%" height={320}>
      <BarChart data={data} margin={{ top: 4, right: 4, bottom: 80, left: 40 }}>
        ...
        <Bar dataKey="value" fill="#e879f9" radius={[4, 4, 0, 0]} maxBarSize={48} />
      </BarChart>
    </ResponsiveContainer>
  );
}
